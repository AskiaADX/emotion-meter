...
function init(optionsADC, ecran) {
    options = {
        start				: optionsADC.start || 50, //Number
        connect			 : optionsADC.connect || [true, false], // false, true, [bool, bool]
    ...
    if (ecran == 1) {
        ...
        //Check if there is an unity passed in the length
        //if not set to px
        if (!options.length.match(/px|%|vw|rem|em/)) {
            options.length += "px";
        }
        //according to it orientation
